...
#    powershell -NoProfile -ExecutionPolicy Bypass -File .\scramble_copy_env.ps1 -WhatIf
#
# 2) Echte Änderung mit Standardlisten:
#    powershell -NoProfile -ExecutionPolicy Bypass -File .\scramble_copy_env.ps1
#
...
foreach ($sourceFile in $EnvFiles) {
    if (-not (Test-Path -LiteralPath $sourceFile)) {
        Write-Warning "Quelldatei nicht gefunden: $sourceFile"
        continue
    }

    $targetFile = "$sourceFile$CopySuffix"
    if ($PSCmdlet.ShouldProcess($targetFile, "Kopie erstellen aus $sourceFile")) {
        Copy-Item -LiteralPath $sourceFile -Destination $targetFile -Force
        Write-Host "Kopiert: $sourceFile -> $targetFile"
    }
...
foreach ($mapping in $fileMappings) {
    $envFile = $mapping.Target
    $readPath = $envFile

    if (-not (Test-Path -LiteralPath $readPath)) {
        if ($WhatIfPreference -and (Test-Path -LiteralPath $mapping.Source)) {
            $readPath = $mapping.Source
        }
        else {
            Write-Warning "Zieldatei nicht gefunden: $envFile"
            continue
        }
    }

    $lines = Get-Content -LiteralPath $readPath
    $updatedInFile = 0
